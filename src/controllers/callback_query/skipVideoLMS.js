...
async function skipVideoLMS({ data, message }) {
    ...
    try {
        ...
        if (isClicked) {
            ...
            const isOK = await page.evaluate(() => new Promise((resolve) => {
                {
                    let time = 0
            ...
            if (!isOK) {
                await browser.close()
                await this.sendMessage(chat_id, `Thử lại sau nhé...`, { reply_message_id: message_id })
                return
            }
                ...
                try {
                    ...
                    function hiddenScreenVideo(tab) {
                        ...
                        if (menuVideo) {
                            ...
                            if (x) {
                                p.textContent = `Đã tua được ${x?.getAttribute("stroke-dasharray")?.split(" ")[0]
                                    }%`;
                            } else {
                    ...
                    async function thanChu() {
                        try {
                            ...
                            while (true) {
                                ...
                                const { status: stausSeeked, message: messageSeeked } = await seekedVideo(
                                    tab,
                                    srcVideo[indexVideo ? 1 : 0]
                                );
                    ...
                    function checkNextTab(tab) {
                        if (tab?.parentElement?.nextElementSibling) {
                            ...
                            if (tab?.parentElement?.nextElementSibling.querySelector("button")) {
                                return {
                                    nextTab: tab?.parentElement?.nextElementSibling.querySelector("button"),
                            ...
                            } else if (!tab.parentElement.parentElement.parentElement.nextElementSibling.querySelector(
                                "button"
                            ) && tab.parentElement.parentElement.parentElement.nextElementSibling.nextElementSibling) {
                                //bỏ qua các trương không có gì
                                if (tab.parentElement.parentElement.parentElement.nextElementSibling.nextElementSibling && !tab.parentElement.parentElement.parentElement.nextElementSibling.querySelector(
                                    "button"
                                )) {
                                    tab = tab.parentElement.parentElement.parentElement.nextElementSibling.nextElementSibling;
                                    while (tab.nextElementSibling.nextElementSibling.tagName === "LI" && !tab.querySelector("button")) {
                    ...
                    function checkProgess(tab) {
                        return new Promise((res) => {
                            res(
                                tab.querySelector("svg.lesson-board__lesson__progress__svg--tick")
                                    ? true
                                    : false
                            );
                        });
